...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    case 'c':
      chains = atoi(optarg);
      if (chains <= 0) {
	fprintf(stderr, "error: no. chains must be 1 or greater\n");
	return -1;
      }
      break;
  ...
  int status = reader.step(model, hierarchical, likelihood);
  int step = 0;
  while (status > 0) {
    ...
    status = reader.step(model, hierarchical, likelihood);
    step ++;
